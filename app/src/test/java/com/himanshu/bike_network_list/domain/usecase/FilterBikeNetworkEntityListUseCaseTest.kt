...
class FilterBikeNetworkEntityListUseCaseTest {
    @get:Rule
    ...
    val bikeNetwork4 = BikeNetworkEntity("4", "Work 4", "url", "City4", "Country4")
    val bikeNetwork5 = BikeNetworkEntity("5", "Work 5", "url", "City5", "Country5")
    ...
    @Test
    fun searchedItemFoundTestCase() {
        val devices = listOf(bikeNetwork2, bikeNetwork1, bikeNetwork3)

        Mockito.`when`(repository.getBikeNetworkList()).thenReturn(testSingle)
        val useCase = FilterBikeNetworkListUseCase(repository)
        val result = useCase("Ne")
    ...
    @Test
    fun searchedItemNotFoundTestCase() {
        val devices = emptyList<BikeNetworkEntity>()

        Mockito.`when`(repository.getBikeNetworkList()).thenReturn(testSingle)
        val useCase = FilterBikeNetworkListUseCase(repository)
        val result = useCase("Xy")
